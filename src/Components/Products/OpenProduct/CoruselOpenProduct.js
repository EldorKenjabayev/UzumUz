...
import "../Product.css";
import "./coruselOpen.css";
export default function CoruselOpenProduct({ setProductInfo, productInfo }) {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        if (productInfo && productInfo.catigoris) {
          setFilteredData(
            res.data.filter((item) => item.catigoris === productInfo.catigoris)
          );
        }
      })
  ...
  return (
    <Box>
      <h3 className="oxshash">Oʻxshash mahsulotlar</h3>
