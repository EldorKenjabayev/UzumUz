...
import './Favorite.css'
export default function Favorite({ setProductInfo }) {
  const [favorite, setFavorite] = useState([]);
  useEffect(() => {
    let favoriteItem = localStorage.getItem("favoriteItems");
    setFavorite(JSON.parse(favoriteItem));
  }, []);
      ...
      {favorite == null || favorite.length === 0  ? (
        ...
        >
          ...
          >
            ...
            />
            <h1 className="TitleFavoriyeEmptyH1" style={{
              textAlign:'center'
            }}> Sizga yoqqanini qoʻshing</h1>
            <h3 className="TitleFavoriyeEmptyH3"
              style={{
                position: "relative",
                top: "-14px",
                textAlign:'center',
            ...
            <Box>
              <Link to={"/"}>
                <button className="BtnFavoriteEpmty"
                  style={{
                    borderRadius: "10px",
                    fontFamily: "Inter",
                    border: "none",
